...
namespace
{
    Point2D ComputeBeamDistanceCreatedNodeWorld(const EditorState& state, const char* beamDistanceBuffer)
    {
        double distance = std::atof(beamDistanceBuffer);
...
BeamDistanceDialogResult AppDialogs::DrawBeamDistanceInputDialog(const EditorState& state)
{
    ...
    if (justOpened)
    {
        ...
        std::snprintf(beamDistanceBuffer, sizeof(beamDistanceBuffer), "%.6g", defaultDistance);
        beamDistanceInputShouldFocus = true;
    }

    result.previewNodeWorld = ComputeBeamDistanceCreatedNodeWorld(state, beamDistanceBuffer);
    ...
    if (ImGui::Begin("Distância###BeamDistanceInput", &keepOpen, flags))
    {
        ...
        ImGui::InputText(
            "##BeamDistanceValue",
            beamDistanceBuffer,
            IM_ARRAYSIZE(beamDistanceBuffer),
        ...
        if (ImGui::Button("Confirmar", ImVec2(buttonWidth, 0.0f)))
        {
            ...
            result.createdNodeWorld = ComputeBeamDistanceCreatedNodeWorld(state, beamDistanceBuffer);
